...
export function InviteMemberDialog({ open, onOpenChange, teamId, teamName }: InviteMemberDialogProps) {
	...
	const handleSubmit = async (e: React.FormEvent) => {
		...
		try {
			...
			const message =
				(err as { data?: { message?: string } })?.data?.message || (err as { message?: string })?.message || "Failed to send invitation";
			toast.error(message);
		}
	};
		...
		<Dialog open={open} onOpenChange={handleOpenChange}>
			<DialogContent className="sm:max-w-md" disableOutsideClick={false}>
				<DialogHeader>
					...
					<DialogDescription>
						Enter the email address of the person you want to invite. They&apos;ll receive an email with a link to join this team.
					</DialogDescription>
				</DialogHeader>

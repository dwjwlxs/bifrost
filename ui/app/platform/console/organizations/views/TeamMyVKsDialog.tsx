...
import { PlatformVirtualKey, usePlatformListTeamMyVKsQuery } from "@/lib/platform/platformApi";

interface TeamMyVKsDialogProps {
	open: boolean;
...
function MyVKRow({ vk }: { vk: PlatformVirtualKey }) {
...
export function TeamMyVKsDialog({ open, onOpenChange, teamId, teamName }: TeamMyVKsDialogProps) {
	const { data: vks, isLoading } = usePlatformListTeamMyVKsQuery(teamId, { skip: !open });

	return (
		<Dialog open={open} onOpenChange={onOpenChange}>
			<DialogContent className="sm:max-w-2xl" disableOutsideClick={false}>
				<DialogHeader>
					<DialogTitle>My Virtual Keys — {teamName}</DialogTitle>
					<DialogDescription>Your own virtual keys in this team (read-only).</DialogDescription>
				</DialogHeader>
				...
				{isLoading ? (
					...
					<div className="overflow-hidden rounded-md border">
						<Table>
							...
							<TableBody>
								{vks.map((vk) => (
									<MyVKRow key={vk.id} vk={vk} />
								))}
							</TableBody>
						</Table>
					</div>
				) : (
					<div className="text-muted-foreground py-8 text-center text-sm">You don&apos;t have any virtual keys in this team.</div>
				)}
			</DialogContent>
		</Dialog>
	);

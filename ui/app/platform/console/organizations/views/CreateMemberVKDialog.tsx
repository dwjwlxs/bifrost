...
import { PlatformTeamMember, usePlatformCreateVKMutation } from "@/lib/platform/platformApi";
...
/** Sentinel values for "no selection" in Radix Select (empty string is not allowed) */
const USER_NONE = "__current_user__";
 ...
 */
export function CreateMemberVKDialog({ open, onOpenChange, teamId, teamName, members, onCreated }: CreateMemberVKDialogProps) {
	const [createVK, { isLoading }] = usePlatformCreateVKMutation();
	const [selectedUserId, setSelectedUserId] = useState(USER_NONE);
	...
	const onSubmit = async (data: CreateVkForm) => {
		try {
			await createVK({
				...
				team_id: teamId,
				user_id: selectedUserId !== USER_NONE ? selectedUserId : undefined,
			...
			const message =
				(err as { data?: { message?: string } })?.data?.message || (err as { message?: string })?.message || "Failed to create VK";
			toast.error(message);
		}
	};
	...
	const handleClose = (value: boolean) => {
		if (!value) {
			reset();
			setSelectedUserId(USER_NONE);
			setSelectedResetDuration(RESET_NONE);
		}
		onOpenChange(value);
	};
		...
		<Dialog open={open} onOpenChange={handleClose}>
			<DialogContent className="sm:max-w-md" disableOutsideClick={false}>
				<DialogHeader>
					...
					<DialogDescription>
						Create a new virtual key for {teamName}. The key will be bound to this team. You can assign it to a team member and set budget
						limits.
					</DialogDescription>
				</DialogHeader>
				...
				<form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
					...
					{members.length > 0 && (
						<div className="bg-muted/50 rounded-md p-3">
							<p className="text-muted-foreground mb-1.5 text-xs font-medium">Team Members ({members.length})</p>
							<div className="flex flex-wrap gap-1.5">
								{members.map((m) => (
									<span key={m.user_id} className="bg-background rounded-full px-2 py-0.5 text-xs">
										{m.username}
									</span>
								))}
							</div>
					...
					<div className="space-y-2">
						...
						<Select value={selectedUserId} onValueChange={setSelectedUserId}>
							...
							<SelectContent>
								<SelectItem value={USER_NONE}>Current User (Team Admin)</SelectItem>
								{members.map((m) => (
						...
						<p className="text-muted-foreground text-xs">Leave empty to create VK for the current team admin</p>
					</div>

					{/* Name */}
					<div className="space-y-2">
						<Label htmlFor="vk-name">Name</Label>
						<Input id="vk-name" placeholder="Production Key" {...register("name")} />
					...
					<div className="space-y-2">
						...
						<Input id="vk-desc" placeholder="Used for production API calls" {...register("description")} />

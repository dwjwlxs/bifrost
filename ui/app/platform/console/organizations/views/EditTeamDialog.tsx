...
import { PlatformTeam, usePlatformUpdateTeamMutation } from "@/lib/platform/platformApi";
...
export function EditTeamDialog({ open, onOpenChange, team, onUpdated }: EditTeamDialogProps) {
	...
	const onSubmit = async (data: EditTeamForm) => {
		try {
			...
			const message =
				(err as { data?: { message?: string } })?.data?.message || (err as { message?: string })?.message || "Failed to update team";
			toast.error(message);
		}
	};
		...
		<Dialog open={open} onOpenChange={handleClose}>
			<DialogContent className="sm:max-w-md" disableOutsideClick={false}>
				<DialogHeader>
					...
					<DialogDescription>
						Update team <strong>{team.name || team.id}</strong>. Changes will apply immediately.
					</DialogDescription>
				</DialogHeader>
				...
				<form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
					...
					<div className="space-y-2">
						...
						<Input id="team-name" placeholder="e.g. Engineering, Product" autoFocus {...register("name")} />
					...
					<div className="space-y-2">
						...
						<p className="text-muted-foreground text-xs">This is a team-wide budget cap. Individual VK budgets are independent.</p>
					</div>

...
func NewPlatformTeamHandler(config *bconfig.BillingPluginConfig) *PlatformTeamHandler {
...
// RegisterRoutes registers all console team routes.
func (h *PlatformTeamHandler) RegisterRoutes(r *router.Router, middlewares ...schemas.BifrostHTTPMiddleware) {
	...
	r.GET("/api/platform/teams", fhttp.ChainMiddlewares(h.listMyTeams, middlewares...))
	r.GET("/api/platform/teams/{teamId}", fhttp.ChainMiddlewares(h.getTeam, teamMemberMw...))
	r.PUT("/api/platform/teams/{teamId}", fhttp.ChainMiddlewares(h.updateTeam, teamAdminMw...))
	r.GET("/api/platform/teams/{teamId}/members", fhttp.ChainMiddlewares(h.listTeamMembers, teamMemberMw...))
	r.POST("/api/platform/teams/{teamId}/members", fhttp.ChainMiddlewares(h.inviteMember, teamAdminMw...))
	r.DELETE("/api/platform/teams/{teamId}/members/{uid}", fhttp.ChainMiddlewares(h.removeMember, teamAdminMw...))
	r.PUT("/api/platform/teams/{teamId}/members/{uid}", fhttp.ChainMiddlewares(h.updateMemberRole, teamAdminMw...))
	r.DELETE("/api/platform/teams/{teamId}", fhttp.ChainMiddlewares(h.deleteTeam, teamAdminMw...))
}
...
func (h *PlatformTeamHandler) listMyTeams(ctx *fasthttp.RequestCtx) {
	...
	// Non-admin users: list teams from their JWT claims (populated from console_team_members)
	...
	teamsByID := make(map[string]configstoreTables.TableTeam, len(teams))
	for _, t := range teams {
		teamsByID[t.ID] = t
	}

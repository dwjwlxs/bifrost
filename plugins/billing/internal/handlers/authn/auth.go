...
func (h *AuthHandler) authMiddleware(next fasthttp.RequestHandler) fasthttp.RequestHandler {
	return func(ctx *fasthttp.RequestCtx) {
		...
		if err != nil {
			handlers.SendError(ctx, fasthttp.StatusUnauthorized, fmt.Sprintf("%d", fasthttp.StatusUnauthorized), "unauthorized: "+err.Error())
			return
		}
...
func mapAuthError(err error) (int, string) {
	...
	case errors.Is(err, authsvc.ErrUserNotVerified):
	...
	case errors.Is(err, authsvc.ErrRefreshTokenExpired):
		return fasthttp.StatusUnauthorized, "refresh token expired"
	case errors.Is(err, authsvc.ErrRefreshTokenUsed):
	...
	case errors.Is(err, authsvc.ErrPasswordResetInvalid):
		return fasthttp.StatusBadRequest, "invalid or expired password reset code"
	case errors.Is(err, authsvc.ErrOAuthProviderDisabled):
	...
	case errors.Is(err, authsvc.ErrNotSessionOwner):
...
func handleServiceError(ctx *fasthttp.RequestCtx, err error) {
	...
	if errors.Is(err, authsvc.ErrUserNotVerified) {
		msg = "email_not_verified"
	}
	handlers.SendError(ctx, code, msg, msg)
}
...
func (h *AuthHandler) register(ctx *fasthttp.RequestCtx) {
	...
	if err := readJSONBody(ctx, &req); err != nil {
		handlers.SendError(ctx, fasthttp.StatusBadRequest, fmt.Sprintf("%d", fasthttp.StatusBadRequest), err.Error())
		return
	}
...
func (h *AuthHandler) refreshToken(ctx *fasthttp.RequestCtx) {
	var req authsvc.RefreshTokenRequest
	if err := readJSONBody(ctx, &req); err != nil {
		handlers.SendError(ctx, fasthttp.StatusBadRequest, fmt.Sprintf("%d", fasthttp.StatusBadRequest), err.Error())
		return
	}
...
func (h *AuthHandler) oauthLogin(ctx *fasthttp.RequestCtx) {
	provider := authsvc.IdentityProvider(ctx.UserValue("provider").(string))
	var body struct {
	...
	tokens, err := h.svc.OAuthLogin(ctx, authsvc.OAuthCallbackRequest{
		Provider: provider,
		Code:     body.Code,
		State:    body.State,
...
func (h *AuthHandler) verifyEmailChange(ctx *fasthttp.RequestCtx) {
	uid := userID(ctx)
	var req authsvc.VerifyEmailChangeRequest
	if err := readJSONBody(ctx, &req); err != nil {
		handlers.SendError(ctx, fasthttp.StatusBadRequest, fmt.Sprintf("%d", fasthttp.StatusBadRequest), err.Error())
		return
	}
...
func (h *AuthHandler) deleteAccount(ctx *fasthttp.RequestCtx) {
	uid := userID(ctx)
	var req authsvc.DeleteAccountRequest
	if err := readJSONBody(ctx, &req); err != nil {
		handlers.SendError(ctx, fasthttp.StatusBadRequest, fmt.Sprintf("%d", fasthttp.StatusBadRequest), err.Error())
		return
	}

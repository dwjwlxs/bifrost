...
func (h *PlatformVKHandler) createVK(ctx *fasthttp.RequestCtx) {
	...
	// Use transaction to ensure atomicity of VK, budget, and default provider configs creation
	err := h.db.Transaction(func(tx *gorm.DB) error {
		...
		// Query all configured providers to create default allow-all provider configs.
		// This ensures the VK can route to any provider for any model by default.
		var providerList []tables.TableProvider
		if err := tx.Find(&providerList).Error; err != nil {

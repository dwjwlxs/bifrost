...
import (
	...
	fpayment "github.com/dwjwlxs/bifrost/plugins/billing/internal/services/payment"
...
// PackageHandler handles billing API endpoints.
...
// adminMw is applied to admin-only endpoints (e.g., RequireAdmin from platform handlers).
func (h *PackageHandler) RegisterRoutes(r *router.Router, middlewares ...schemas.BifrostHTTPMiddleware) {
	adminMw := RequireAdmin

	// Balance
	r.GET("/api/billing/balance", fhttp.ChainMiddlewares(h.handleBalance, middlewares...))

	// Recharge
	r.POST("/api/billing/recharge", fhttp.ChainMiddlewares(h.createRecharge, middlewares...))
	r.POST("/api/billing/admin/recharge", fhttp.ChainMiddlewares(h.adminRecharge, append(middlewares, adminMw)...))

	// Package CRUD
	r.GET("/api/billing/packages", fhttp.ChainMiddlewares(h.listPackages, middlewares...))
	r.GET("/api/billing/packages/{packageId}", fhttp.ChainMiddlewares(h.getPackage, middlewares...))
	r.POST("/api/billing/packages", fhttp.ChainMiddlewares(h.createPackage, append(middlewares, adminMw)...))
	r.PUT("/api/billing/packages/{packageId}", fhttp.ChainMiddlewares(h.updatePackage, append(middlewares, adminMw)...))
	r.DELETE("/api/billing/packages/{packageId}", fhttp.ChainMiddlewares(h.deletePackage, append(middlewares, adminMw)...))

	// Purchase
	r.POST("/api/billing/purchases", fhttp.ChainMiddlewares(h.createPurchase, middlewares...))
	r.POST("/api/billing/admin/purchase", fhttp.ChainMiddlewares(h.adminPurchase, append(middlewares, adminMw)...))

	// Entity Packages
	r.GET("/api/billing/entity-packages", fhttp.ChainMiddlewares(h.listEntityPackages, middlewares...))
	r.GET("/api/billing/entity-packages/{epId}", fhttp.ChainMiddlewares(h.getEntityPackage, middlewares...))

	// User Packages (computed view of entity packages with usage)
	r.GET("/api/billing/user/packages", fhttp.ChainMiddlewares(h.listUserPackages, middlewares...))

	// Orders
	r.GET("/api/billing/orders", fhttp.ChainMiddlewares(h.listOrders, middlewares...))
	r.GET("/api/billing/orders/{orderId}", fhttp.ChainMiddlewares(h.getOrder, middlewares...))

	// Generic gateway webhook — routes based on URL path
	r.POST("/api/billing/webhook/{gateway}", h.handleWebhook)

	// Order cancellation
	r.POST("/api/billing/orders/{orderId}/cancel", fhttp.ChainMiddlewares(h.cancelOrder, middlewares...))
	...
	r.POST("/api/billing/orders/{orderId}/confirm", fhttp.ChainMiddlewares(h.confirmOrder, append(middlewares, adminMw)...))
}
...
func (h *PackageHandler) handleBalance(ctx *fasthttp.RequestCtx) {
...
func (h *PackageHandler) createRecharge(ctx *fasthttp.RequestCtx) {
	claims := GetPlatformClaimsFromContext(ctx)
	if claims == nil {
		SendError(ctx, fasthttp.StatusUnauthorized, "UNAUTHORIZED", "user_id is required")
		return
...
func (h *PackageHandler) adminRecharge(ctx *fasthttp.RequestCtx) {
...
func (h *PackageHandler) listPackages(ctx *fasthttp.RequestCtx) {
...
func (h *PackageHandler) getPackage(ctx *fasthttp.RequestCtx) {
...
func (h *PackageHandler) createPackage(ctx *fasthttp.RequestCtx) {
...
func (h *PackageHandler) updatePackage(ctx *fasthttp.RequestCtx) {
	packageID := ctx.UserValue("packageId").(string)
...
func (h *PackageHandler) deletePackage(ctx *fasthttp.RequestCtx) {
	packageID := ctx.UserValue("packageId").(string)
...
func (h *PackageHandler) createPurchase(ctx *fasthttp.RequestCtx) {
	claims := GetPlatformClaimsFromContext(ctx)
	if claims == nil {
...
func (h *PackageHandler) adminPurchase(ctx *fasthttp.RequestCtx) {
...
func (h *PackageHandler) listEntityPackages(ctx *fasthttp.RequestCtx) {
	claims := GetPlatformClaimsFromContext(ctx)
	if claims == nil {
		SendError(ctx, fasthttp.StatusUnauthorized, "UNAUTHORIZED", "Authentication required")
		return
...
func (h *PackageHandler) listUserPackages(ctx *fasthttp.RequestCtx) {
	claims := GetPlatformClaimsFromContext(ctx)
	if claims == nil {
		SendError(ctx, fasthttp.StatusUnauthorized, "UNAUTHORIZED", "Authentication required")
		return
	...
	budgetMap := make(map[string]configstoreTables.TableBudget, len(budgetIDSet))
...
func (h *PackageHandler) getEntityPackage(ctx *fasthttp.RequestCtx) {
	claims := GetPlatformClaimsFromContext(ctx)
	if claims == nil {
		SendError(ctx, fasthttp.StatusUnauthorized, "UNAUTHORIZED", "Authentication required")
		return
...
func (h *PackageHandler) listOrders(ctx *fasthttp.RequestCtx) {
	claims := GetPlatformClaimsFromContext(ctx)
	if claims == nil {
		SendError(ctx, fasthttp.StatusUnauthorized, "UNAUTHORIZED", "user_id is required")
		return
...
func (h *PackageHandler) getOrder(ctx *fasthttp.RequestCtx) {
	claims := GetPlatformClaimsFromContext(ctx)
	if claims == nil {
		SendError(ctx, fasthttp.StatusUnauthorized, "UNAUTHORIZED", "user_id is required")
		return
...
func (h *PackageHandler) handleWebhook(ctx *fasthttp.RequestCtx) {
...
func (h *PackageHandler) cancelOrder(ctx *fasthttp.RequestCtx) {
	claims := GetPlatformClaimsFromContext(ctx)
	if claims == nil {
		SendError(ctx, fasthttp.StatusUnauthorized, "UNAUTHORIZED", "Authentication required")
		return
...
func (h *PackageHandler) confirmOrder(ctx *fasthttp.RequestCtx) {
...
func (h *PackageHandler) retryPay(ctx *fasthttp.RequestCtx) {
	claims := GetPlatformClaimsFromContext(ctx)
	if claims == nil {
		SendError(ctx, fasthttp.StatusUnauthorized, "UNAUTHORIZED", "Authentication required")
		return
...
func (h *PackageHandler) listGateways(ctx *fasthttp.RequestCtx) {
	claims := GetPlatformClaimsFromContext(ctx)
	if claims == nil {
		SendError(ctx, fasthttp.StatusUnauthorized, "UNAUTHORIZED", "Authentication required")
		return
